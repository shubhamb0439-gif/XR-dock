...
function handleSignalMessage(data) {
    ...
    if (type === 'drug_availability' || type === 'drug_availability_console') {
        ...
        // Log each drug's availability status
        results.forEach(item => {
            const status = item.status === 'exists' ? '✓ AVAILABLE' : '✖ NOT FOUND';
            console.log(`[DRUG_AVAILABILITY] ${item.query}: ${status}`, item.matched ? `(matched as: ${item.matched})` : '');
        });
